...
- **SQLite** один файл (`data/potracheno.db`), том в Docker. Схема — `Base.metadata.create_all`
  в `init_db()` + точечная идемпотентная миграция в `_migrate` (PRAGMA + ALTER TABLE для SQLite);
  для крупных изменений схемы — пересоздать БД или добавить Alembic.
...
- `bot/db/models.py` — `User`, `Payment`, `ReminderLog`, `Feedback`, `PaymentMethod`
  (+ enum'ы `Freq`, `PaymentStatus`, `ReminderStatus`, `FeedbackKind`).
  - `Payment.payment_method` — способ оплаты, хранится **строкой** (`String(64)`, nullable;
    `NULL` = не указан), а НЕ FK. Значение — `«Наличные»`, `«Карта/трансфер»` или название
    своего способа (`PaymentMethod.name`). Колонка добавляется идемпотентной миграцией в
    `init_db` (`_migrate`: `PRAGMA table_info` + `ALTER TABLE`), т.к. SQLite-`create_all`
    не меняет существующие таблицы.
  - `PaymentMethod` — пользовательские способы оплаты (этап 1), лимит `MAX_PAYMENT_METHODS=10`.
